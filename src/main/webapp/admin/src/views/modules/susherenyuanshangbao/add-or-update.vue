<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="缺席人数" prop="quexirenshu">
          <el-select v-model="ruleForm.quexirenshu" placeholder="请选择缺席人数">
            <el-option
                v-for="(item,index) in quexirenshuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="缺席人数" prop="quexirenshu">
	      <el-input v-model="ruleForm.quexirenshu"
                placeholder="缺席人数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="宿舍长账号" prop="sushezhangzhanghao">
          <el-input v-model="ruleForm.sushezhangzhanghao" 
              placeholder="宿舍长账号" clearable  :readonly="ro.sushezhangzhanghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="宿舍长账号" prop="sushezhangzhanghao">
              <el-input v-model="ruleForm.sushezhangzhanghao" 
                placeholder="宿舍长账号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='quexirenshu'){
            this.ruleForm.quexirenshu = obj[o];
	    this.ro.quexirenshu = true;
            continue;
          }
	            	            if(o=='sushezhangzhanghao'){
            this.ruleForm.sushezhangzhanghao = obj[o];
	    this.ro.sushezhangzhanghao = true;
            continue;
          }
	            	            if(o=='sushehao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
                                                          this.ruleForm.sushezhangzhanghao = json.sushezhangzhanghao
          ...
          this.$message.error(data.msg);
                                          ...
                                          this.quexirenshuOptions = "0,1,2,3,4".split(',')
                                                ...
                                                // 多级联动参数
                                                              info(id) {
      this.$http({
        url: `susherenyuanshangbao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `susherenyuanshangbao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.susherenyuanshangbaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
